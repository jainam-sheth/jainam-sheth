        ...
        <Card className="border-border bg-card">
          <CardContent className="p-0">
            ...
            {lobData.map((data, idx) => (
              <div
                key={data.label}
                className={`grid grid-cols-[200px_1fr_1.6fr_1.6fr] items-stretch ${idx < lobData.length - 1 ? "border-b border-border" : ""} ${data.isTotal ? "bg-muted/30" : ""}`}
                style={{ minHeight: "150px" }}
              >
                {/* LOB label */}
                <div className="pr-3 py-5 flex items-center">
                  <h5 className={`text-sm leading-snug ${data.isTotal ? "font-extrabold" : "font-bold"} text-foreground`}>
                    {data.label}
                  </h5>
                </div>
